...
var routes = new Dictionary<string, RouteConfig>(StringComparer.OrdinalIgnoreCase)
{
    ...
    ["/api/b"] = new RouteConfig(
        ...
        AllowAnonymousPrefixes: ["/health"],
        RequestsPerWindow: int.TryParse(Environment.GetEnvironmentVariable("RATE_LIMIT_API_A"), out var rlb) ? rlb : throw new Exception("RATE_LIMIT_API_A not set in .env"),
        Window: TimeSpan.FromMilliseconds(int.TryParse(Environment.GetEnvironmentVariable("RATE_WINDOW_API_A_MS"), out var rwb) ? rwb : throw new Exception("RATE_WINDOW_API_A_MS not set in .env"))
    ),
};
